...
exports.createPost = async (req, res) => {
    ...
    //   inserting at the end of the user's posts array
...
// only logged in user can delete his post
...
// deleting the post from user and post collection
      const user = await User.findById(req.user._id);
  ...
  exports.updateCaption = async (req, res) => {
    try {
      ...
      // only user can change their caption
      if (post.owner.toString() !== req.user._id.toString()) {
        return res.status(401).json({
          success: false,
          message: "Unauthorized",
  ...
  exports.commentOnPost = async (req, res) => {
    try {
      ...
      if (commentIndex !== -1) {
        // updating the comment at that particular index of the user
  ...
  exports.deleteComment = async (req, res) => {
    try {
      ...
      if (post.owner.toString() === req.user._id.toString()) {
        ...
        // when user deleting their comment
        post.comments.forEach((item, index) => {
          if (item.user.toString() === req.user._id.toString()) {
            return post.comments.splice(index, 1);
          }
